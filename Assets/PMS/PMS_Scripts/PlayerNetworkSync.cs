...
public class PlayerNetworkSync : MonoBehaviourPun
{
    ...
    private void OnEnable()
    {
        ...
        if (_gamePlayer != null)
        {
            _gamePlayer.OnHpChanged += OnGamePlayerHpChanged;
        }
    }
    ...
    private void OnGamePlayerHpChanged(int currentHp, bool isAlive)
    {
        ...
        if (photonView.IsMine)
        {
            ...
            //현재 체력과 현재 생존 상태를 전달
            photonView.RPC("RPC_UpdatePlayerState", RpcTarget.All, currentHp, isAlive);
            Debug.Log($"[PlayerNetworkSync] 내 HP 변경 RPC 호출: CurrentHp : {currentHp}, IsAlive : {isAlive}");
        }
    }

    //플레이어의 MaxHp변동에 따른 hp값과 그에 따른 IsAlive값 동기화를 위한 RPC함수 - MaxHp를 조정한다는 것은 라운드가 끝나서 초기화 되야 한다는 것을 의미
    //모든 플레이어들을 살려내야한다.
    [PunRPC]
    public void RPC_UpdatePlayerState(int newMaxHp, bool newIsAlive, PhotonMessageInfo info)
    {
        ...
        Debug.Log($"[PlayerNetworkSync] RPC 수신: {info.Sender.NickName} ({_gamePlayer.PlayerId})의 상태 업데이트: HP : {_gamePlayer.CurrentHp}, Alive : {_gamePlayer.IsAlive}");
    }
